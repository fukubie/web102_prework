...
function addGamesToPage(games) {
    ...
    for(let i=0;i<games.length; i++){
        ...
        const gameCard = document.createElement(/* some HTML tag here */"div");

        // add the class game-card to the list
        gameCard.classList.add(/* string with class name here */"game-card");
...
const totalCtr=GAMES_JSON.reduce((sum,game) => {
    return sum+game.backers;
},0);

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `$${totalCtr.toLocaleString()}`;
